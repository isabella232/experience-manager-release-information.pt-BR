...
* As propriedades de uma página principal não são carregadas e uma exceção de ponteiro nulo é retornada. O problema é resolvido ao adicionar a propriedade cq:blueprint (NPR-30901).
...
* Pronto para uso, o Rich Text Editor (RTE) de andaime. aplica o tamanho da fonte incorporado a elementos, inesperadamente (NPR-31283, NPR-30922).
* Não é possível sincronizar a campanha em Adobe que contém o componente de importador de design pronto para uso (NPR-30890).
* Rich Text Editor (RTE). O não permite inserir uma Tabela incorporada como um item de lista (NPR-30878).
...
* Os relatórios do Analytics não são preenchidos no ambiente do autor do AEM. Uma página em branco é exibida (NPR-30905).
* A paginação não está funcionando corretamente nos blogs das comunidades (NPR-30827).
...
* A geração da visualização HTML5 de um formulário XDP exibe uma cintilação ao adicionar instâncias de um subformulário (NPR-30908).

### Instalador do Forms JEE {#forms-jee-installer}

#### Serviços de documento {#document-services}

* O OutputService exibe uma resposta incorreta após a aplicação de um patch para corrigir problemas de HTML para PDF (NPR-31504).
...
* O processamento de subativos de arquivos de PDF grandes causa uma exceção OutOfMemoryError (OOME). NPR-29851: Hotfix do CQ-4269574
...
* O valor do parâmetro de solicitação resourceType é copiado para o valor de um atributo de tag HTML que é encapsulado em aspas duplas. NPR-29832: Hotfix do CQ-4255365
...
* Quando um teste de carga é executado no serviço HTML para PDF, ele falha com um erro e as configurações de tipo de arquivo são removidas do servidor de formulários AEM. NPR-30111, NPR-30086: Hotfix do CQ-4271495
...
* Atualização da interface DAM DMGateway para suporte a várias partes do S3. NPR-29740: Hotfix do Q-4226303
...
* O proprietário não pode criar uma pasta particular dentro de uma pasta particular. NPR-29397: Hotfix do CQ-4229830
* O upload de um arquivo do Adobe Illustrator Artwork com mais de 2 GB gera um erro de espaço de heap Java. NPR-29265: Hotfix do CQ-4226217
...
* O compartilhamento de link de ativo exibe cartões cinza em branco para alguns ativos na pasta. NPR-29831: Hotfix do CQ-4270187
...
* Problema ao tentar comparar a versão mais recente e atual da página no AEM 6.3.3.3. NPR-29532: Hotfix do CQ-4269639
...
* Minimizar o Editor de Rich Text na tela cheia enquanto estiver no modo de edição de origem resulta em perda de conteúdo. NPR-28838: Hotfix do CQ-4260584
...
* Um alto tempo de resposta de até 40-50 segundos é observado no logon do usuário para a Comunidade. NPR-29679: Hotfix do CQ-4269444
...
* Adição do tempo limite do soquete e do tempo limite da conexão nos agentes de replicação do Brand Portal.
...
* Os valores são diferentes entre a página de coleção e a página de coleção usando o filtro. NPR-28642: Hotfix do CQ-4261405
* CommitFailedException aciona com o upload de ativos de arquivo compactados grandes. NPR-28528: Hotfix do CQ-4260903
* Falha ao salvar metadados de pasta ao editar uma pasta que contém caracteres especiais. NPR-28211: Hotfix do CQ-4260401
...
* A entrega de vídeo de desktop DMS7 usando DMComponent usa download progressivo para reproduzir vídeo no modo de publicação e não faz stream. NPR-28754: Hotfix do CQ-4263732
* Não é possível criar/editar predefinições de imagem, pois a restrição de acesso a /etc/dam/video/dynamicmedia desativa recursos para Gerenciadores de imagem. NPR-28662: Hotfix do CQ-4263022
* O compartilhamento de links com arquivos de vídeo codificados DMS7 resulta em pastas vazias. NPR-28851: Hotfix do CQ-4206743
* A replicação do AEM para o Brand Portal fica paralisada por longos períodos. NPR-28913: Hotfix do CQ-4254932

### Communities {#communities-3}

* Não é possível abrir mensagens com anexos enviados no Outlook e na pasta da caixa de entrada. NPR-28559: Hotfix do CQ-4217072

### Sites {#sites-4}

* Criação de script entre sites (XSS) no SuggestionHandler para 6.3. NPR-28692: Hotfix do CQ-4253821
* A implantação profunda termina sem conter todas as ramificações na respectiva Live Copy. NPR-29175: Hotfix do CQ-4239472
...
* O uso da credencial do Brightedge falha com erro de conexão. NPR-29167: Hotfix do CQ-4265872
* Problema de compilação em OfferproxyTandtProvider.java devido à ausência da instrução de importação para a classe &quot;Recurso&quot;: falta a instrução de importação: import org.apache.sling.api.resource.Resource. NPR-28772
...
* O repositório integrado (Apache Jackrabbit Oak) foi atualizado para a versão 1.6.16.
* A paginação da lista de configuração de política muda para limitar 50 registros por página.
...
* Incluído um caractere de escape para a vírgula quando qualquer pesquisa for executada.
* Suporte ativado de recursos sintéticos para a política de conteúdo.
...
* Ativação da navegação pelo teclado para componentes de Ativação do AEM Communities na Publicação. NPR-27739: Hotfix do CQ-4253856
* Ativação da navegação pelo teclado para acionar a reprodução de conteúdo. NPR-27738: Hotfix do CQ-4254026
...
* Os comentários publicados na próxima página ficam destacados com amarelo quando um novo comentário é publicado. NPR-28148: Hotfix do CQ-4259681
* Não é possível abrir mensagens com anexos enviados no Outlook e na pasta da caixa de entrada. NPR-28559: Hotfix do CQ-4217072
...
* O plug-in de estilo não tem suporte no modo de tela cheia do Editor de Rich Text. NPR-27622: Hotfix do CQ-4258674
...
* O destaque da anotação desaparece assim que o usuário clica na anotação. BPR-27196: Hotfix do CQ-4254423
...
* O cancelamento da herança da Live Copy não funciona corretamente em contêineres direcionados, pois nenhuma ação é exibida. NPR-28129: Hotfix do CQ-4259813

#### Replicação {#replication-1}

* Replicação de falha do DispatcherFlushRules na 6.3.3.1 NPR-28150: Hotfix do CQ-4261401
...
* O download por email de compartilhamento de link retorna um zip vazio/corrompido. Hotfix do CQ-4259686
...
* Não é possível renderizar PDF forms no espaço de trabalho HTML5. NPR-28059: Hotfix do CQ-4260373
...
* Ativação da notificação unreadCount para ser configurada com qualquer número. Valor padrão definido como 20.
...
* Melhore a consulta para usar o recurso Notificação de Expiração. NPR-26999: Hotfix do CQ-4251188
...
* A herança de configuração de Cloud Service está sendo interrompida ao editar outras propriedades de página. NPR-27311: Hotfix do CQ-4256785
...
* A página aparece em branco quando o usuário tenta editar o fragmento de conteúdo. NPR-27669
* Não foi possível criar uma versão para os Fragmentos de experiência. NPR-27689: Hotfix do CQ-4259009
...
* Quando data-sly-use é usado com classes com um nome idêntico, o código não compatível é produzido. NPR-27282: Hotfix do Sling-7581
...
* Uma imagem não é exibida após especificar pontos de interrupção no componente de mídia dinâmica. Hotfix do CQ-4256168
...
* A interface do esquema de metadados da pasta está quebrando após a instalação do SP3. Hotfix do CQ-4257737

#### Communities {#communities-5}

* Adicionar suporte à paginação para a listagem de grupo na publicação. NPR-26953: Hotfix do CQ-4246525
* A notificação de contagem de itens não lidos não pode ser definida além de 21. NPR-27496: Hotfix do CQ-4252829
* O limite de Assinaturas de Usuários é de 1000. NPR-27615: Hotfix do CQ-4254689
...
* As relações com UGC não são excluídas após a exclusão de UGC. NPR-27631: Hotfix do CQ-4258706
* Não é possível pesquisar pelo valor do endereço na Pesquisa de palavra-chave se a comunidade estiver definida com o Provedor de recurso de armazenamento de banco de dados (DSRP). NPR-27652: Hotfix do CQ-4253261
...
* As publicações e respostas do fórum são adicionadas sobre a segunda página e, quando atualizada, a publicação é movida para o local correto na parte superior da primeira página. NPR-27341: Hotfix do CQ-4247338
...
* Ao ativar &#39;Permitir privilegiados&#39;, os membros privilegiados só devem poder compor para usuários que sejam membros da comunidade. NPR-27901: Hotfix do CQ-4251235
...
* Correções proativas do platform.login. NPR-26961
...
* Problema de simultaneidade com o JEE Client SDK. NPR-27572: Hotfix do CQ-4247156
...
* O componente de Contêiner do AEM Forms fica invisível assim que o formulário errado é configurado na página de sites e a caixa de seleção &quot;formulários cobrem toda a largura da página&quot; é marcada. NPR-25972: Hotfix do CQ-4239287, CQ-4249133
...
* Correção de um problema no tratamento FormData para o valor padrão.
* FormBuilder atualizado para a versão mais recente do Handlebars.
* Adição do servlet de configuração para a configuração de edição do RTE no modo de diálogo.
...
* Não é possível abrir a descompactação de um arquivo criado pela etapa de descompactação com uma pasta que tenha um % no nome. NPR-26275: Hotfix do CQ-4251482
...
* O compartilhamento do link do ativo de várias IDs de locatário precede a ID de locatário formando um URL inválido. NPR-26482: Hotfix do CQ-4253540
...
* Problema de utilização da memória de heap com a geração de miniaturas. NPR-25545: Hotfix do CQ-4246960
...
* Não é possível excluir valores padrão para campos de vários valores. NPR-25116: Hotfix do CQ-4247856
* (DM Hybrid) A replicação de catálogo falha no AEM 6.3.2 com Dynamic Media. NPR-26406: Hotfix do CQ-4251306

#### Sites {#sites-7}

* Correções do Backport pró-ativo de sites. NPR-26963
* As tags são criadas duas vezes quando há uma diferença no tipo de letra maiúscula de Título e Nome. NPR-26877: Hotfix do CQ-4254134
...
* Perguntas sobre armazenamento em cache do ClientContext segment.js versus perguntas sobre não armazenamento em cache. NPR-26622: Hotfix do CQ-4253486
...
* Promover um lançamento de várias páginas cria várias versões para cada página. NPR-26810: Hotfix do CQ-4254663
...
* O TSDK incorporado com AEM é interrompido com uma atualização para o Handlebars 4 devido ao uso de modelos incompatíveis. NPR-26699: Hotfix do CQ-4248974
...
* Download redundante de conteúdo de imagem extra dos servidores S7. Hotfix do CQ-4248770

#### Granite {#granite}

* O provedor OAuth de AEM não executa a pesquisa que não diferencia maiúsculas de minúsculas. NPR-26133: Hotfix do GRANITE-22650
* O Validador de pacotes não valida pacotes incluídos no CFP/SP. NPR-26775: Hotfix do Granite-22825

#### Communities {#communities-6}

* Problema do delimitador com resultados da pesquisa. NPR-27051: Hotfix do CQ-4248939
...
* A notificação &quot;Marcar tudo como lido&quot; renderiza apenas os primeiros 10 como não lidos sem atualização da página. NPR-27036: Hotfix do CQ-4254058
* Comportamento de Clique e Carregamento de página da seção de comentários das comunidades. NPR-27030: Hotfix do CQ-4251228
* (Pesquisa no fórum) O botão Voltar ignora a página e transfere de volta para o forum.html. NPR-26949: Hotfix do CQ-4254804
* A notificação de itens não lidos não aumenta mais de 21. NPR-26947: Hotfix do CQ-4251251
* (Trabalho SearchScheduledPosts) Adicione o switch ativar/desativar no ConfigMgr. NPR-26924: Hotfix do CQ-4250463
* Contexto do Tomcat (/aempublish) O caminho cai ao rolar a página Atribuições. NPR-26919: Hotfix do CQ-4254345
* NullPointerException ao criar um grupo e membro. NPR-26778: Hotfix do CQ-4248095
* O desafixação do moderador e o conteúdo sem recursos não funcionam com o Princípio de responsabilidade única (SRP) do MySQL. NPR-26767: Hotfix do CQ-4251520
* Problemas de funcionalidade de pesquisa com determinados caracteres. NPR-26726: Hotfix do CQ-4247744
* Habilite deep links para a interface do usuário de moderação e recursos de ativação. NPR-26705: Hotfix do CQ-4251381
* Problema de pesquisa no componente do fórum. NPR-26577: Hotfix do CQ-4251303
* Pesquisar com o nome e sobrenome juntos em mensagens de comunidades não retorna os resultados esperados. NPR-26377: Hotfix do CQ-4253150
* A paginação não é atualizada após a remoção de respostas. NPR-26327
...
* As configurações de grupo não são carregadas em um grupo que contém 50 mil a 100 mil usuários. NPR-25642: Hotfix do CQ-4238426
* Falha do reprodutor de recursos de catálogo em caminhos de contexto. NPR-25640: Hotfix do CQ-4238426
...
* Correções de logon da plataforma proativa. NPR-26961
...
* Correções proativas do CQ/Experience Log. NPR-26943
* Backports da interface do usuário do Proactive Foundation. NPR-26942
...
* Exceções de Substituição de Chave de Entidade nos logs do servidor para chaves inativas. NPR-26748: Hotfix do CQ-4253705
...
* A validação PDF/A não mostra um válido com &quot;validate PDF/A. NPR-25934: Hotfix do CQ-4248558

#### Forms - Comunicação interativa {#forms-interactive-communication}

* A Visualização de PDF e HTML da Carta é visível e funcional na mesma janela quando um módulo editável é editado, salvo e, em seguida, a Visualização de PDF é aberta/fechada. NPR-26770: Hotfix do CQ-4253217
...
* Exceção de ponteiro nulo é lançada quando o ponto inicial associado do formulário/conjunto de formulários adaptável não pré-preenchido é aberto. Hotfix do CQ-4256124
...
* O rascunho da tarefa/ponto de início está falhando no servidor JEE 6.4 mais recente. Hotfix do CQ-4241484
...
* Revisão do código da API de segurança corporativa. Hotfix do CQ-4255638
* Falha ao carregar esapi.properties como recurso de carregador de classe em WAS9. Hotfix do CQ-4255631
...
* Correção dos problemas relatados na análise de código estático do Core. Hotfix do CQ-104446
...
* org.jgroups.Message nos logs após a atualização do LiveCycle para os formulários AEM 6.2. NPR-26795: Hotfix do CQ-4220415
* Formulários AEM - Erro ao migrar estilos. Hotfix do CQ-4251969
...
* Ativados subativos e visualizador de ativos de várias páginas no Brand Portal.
...
* Mensagem de erro configurada do servidor quando uma imagem viola os critérios de carregamento.
...
* O campo Grupo de usuários fechado (CUG) da pasta de ativos não retorna o grupo &quot;todos&quot;. NPR-23163: Hotfix do CQ-4239377
* A pesquisa por pesquisas salvas de coleções inteligentes não retorna todos os resultados. NPR-23243: Hotfix do CQ-4240355
...
* Não é possível anotar um ativo de vídeo MP4 de única representação no Dynamic Media S7 (DMS7). NPR-22046: Hotfix do CQ-4215912
* Os dados EmbedXMP são sempre definidos como &quot;ativos&quot; para o processo de geração de Ptiff. NPR-22903: Hotfix do CQ-4234498
* Problemas de seleção/exibição de Representação dinâmica com uma grande contagem de Predefinições de imagem. NPR-23151: Hotfix do CQ-4217511
...
* Parsys quebra a largura do quadro do dispositivo emulador e corta qualquer componente adicionado nele. NPR-22926: Hotfix do CQ-4238224
...
* O Editor de modelos exibe apenas 20 modelos/pastas e não carrega os outros ao rolar a tela para a parte inferior da página. NPR-23185: Hotfix do CQ-4238483
...
* A interface clássica não funciona para editar o CUG devido à desativação. NPR-24122: Hotfix do 4241823
* Correção proativa para proteger contra promoções de conteúdo indesejadas. NPR-24387: Hotfix do 4244993
* Uma vez aprox. Adicionamos 80 fragmentos em uma pasta no Assets, erros encontrados quando o fluxo de trabalho é acionado no console Linha do tempo. NPR-23393; Hotfix do CQ-4211216
...
* As opções de sistema de estilo aparecem em todas as páginas após a instalação da 6.3.2.1. NPR-23972: Hotfix do CQ-4244394
* A interface clássica não funciona para editar o CUG devido à desativação. NPR-24122: Hotfix do 4241823
* Correção proativa para proteger contra promoções de conteúdo indesejadas. NPR-24387: Hotfix do 4244993
...
* A coleção inteligente não exibe os resultados após salvar se estiver usando o predicado de tags na faceta de pesquisa. NPR-23401: Hotfix do Granite-21278
...
* A tabela do Foundation está vulnerável a Scripts armazenados entre sites. NPR-23214: Hotfix do CQ-4240760
...
* DatePicker não dá suporte à dica de tipo externo definida manualmente por campo oculto. A alteração da dica de tipo obtém um erro de conversão. NPR-23371: Hotfix do Granite-21194
* Coral.ColumnView: adicione suporte para shift+click. NPR-23404: Hotfix do Granite-13338
...
* As conexões de rastreamento de cliente Apache Http e a alta utilização de heap causam falha no servidor AEM. NPR-23906: Hotfix do Granite-21056
...
* O upload de uma imagem para um Fórum das comunidades não é exibido no Adobe Storage Resource Provider (ASRP). NPR-23397: Hotfix do CQ-4242497
* As ideias excluídas são exibidas como links ativos nas Atividades. NPR-23406
* imsmanifest.xml não pode ser carregado com AEM em execução com raiz de contexto. NPR-23483: Hotfix do CQ-4242193
...
* Problemas com componentes de comunidades quando acessados por meio do dispatcher e do sling dynamic incluem a ativação de. NPR-23586: Hotfix do CQ-4242360, CQ-4241522
* Ao pesquisar por um termo de pesquisa que produz muitos resultados por meio da pesquisa e, em seguida, inserir um novo termo de pesquisa, a paginação não é redefinida. NPR-23739: Hotfix do CQ-4222593
...
* Nenhum detalhe sobre os usuários do sinalizador e os motivos são exibidos usando a configuração DSRP. NPR-23973: Hotfix do CQ-4243205
...
* Não é possível armazenar anexos com tipos MIME com mais de 15 caracteres usando o Desenvolvedor de Recurso de Armazenamento do Banco de Dados (DSRP). NPR-24174
* Quando uma imagem que viola os critérios de carregamento é arrastada e solta no texto da publicação, um erro de servidor é lançado e uma mensagem de erro genérica é exibida ao usuário. NPR-24243
* Correções para vários problemas do lado do servidor do AEM Communities. NPR-23971: Hotfix do CQ-4243144, CQ-4242145, CQ-4243365, CQ-4244098
* Correções para vários problemas do Adobe Social. NPR-24242: Hotfix do CQ-4245054, CQ-4245120, CQ-4245296
...
* A operação do LiveCopy não ignora nós filhos de componentes excluídos. NPR-23058: Hotfix do CQ-4211662
...
* Mover ReCaptchaConfigService para o pacote interno. Hotfix do CQ-4217459
...
* Para incluir SDLInvokerParams no SDK do cliente do complemento Forms. NPR-23157
...
* LiveCycle vulnerável a XSS no PMAdmin. NPR-23267
...
* O serviço de captura de papel trava durante o processamento simultâneo de arquivos TIFF (tamanho de aproximadamente 50 KB ). NPR-23556

#### Designer do Forms {#forms-designer}

* Saída do servidor do AEM Forms - Descrição alternativa ausente para anotações. NPR-22207
* Adicionar suporte de PDF/UA a formulários XML gerados pelo Designer e pelo Serviço de saída. NPR-23132
...
* Melhoria na renderização dos componentes do Parsys na interface clássica.
* Os pacotes de modelos do sling foram atualizados para incluir a correção de conjunto de caracteres.
* Publicação no Brand Portal assíncrona para todos os tipos de ativos.
* Atualização de coralui-component-richtexteditor.git de 0.1.15 para 0.1.16
* Correções na funcionalidade mostrar/ocultar do componente suspenso.
* Ativação do fluxo de imagem para o Componente de imagem principal.
...
* A interface do usuário gera o erro falso &quot;Falha ao publicar no Brand Portal&quot;, enquanto o trabalho é adicionado à fila Replicação e os ativos são publicados no Brand Portal. NPR-22179: Hotfix do CQ-4205273
...
* Falha ao tentar descompactar um arquivo ZIP devido a um espaço à direita no final do nome da pasta. NPR-22522: Hotfix do CQ-4238036
* A classificação usando a coluna Título de ativo não funciona para os resultados da pesquisa. NPR-22908: Hotfix do CQ-4239076
...
* Os ativos errados são excluídos se a pasta selecionada for alterada entre o momento em que os 2 pop-ups são exibidos. NPR-21980: Hotfix do CQ-4233675
...
* Opção Tornar compartilhamento público como padrão ao publicar ativos no Brand Portal. NPR-21931: Hotfix do CQ-4218816
...
* Componentes de estrutura editáveis perdem nomes de classe CSS necessários para a grade responsiva quando editados. NPR-21741: Hotfix do CQ-4232374
...
* RTE Tela cheia A caixa de diálogo da interface para toque está ocultando os plug-ins do RTE. NPR-22034: Hotfix do CQ-4235457
* (Interface de toque) O Editor de Rich Text remove todos os atributos exceto a ID da tag &lt;a>. NPR-22044: Hotfix do CQ-4234133
* Vários Parsys empilhados devido a consultas de longa execução (mais de 6) tornam o AEM lento. NPR-22134: Hotfix do CQ-4233904
...
* Fazer o fallback de TreeNode para o atributo de nome quando o texto estiver vazio. NPR-22146: Hotfix do CQ-4234724/CQ-4236300
* Problemas de RSS Feed, porta -1 para AEM 6.3. NPR-22176: Hotfix do CQ-4233339
...
* Inconsistência no destaque do Trecho retornado pela Pesquisa Lucene. NPR-22879: Hotfix do CQ-4238513
...
* deadlock do agendador durante a inicialização. NPR-22440: Hotfix do Sling-7004
* Não é possível sincronizar grupos entre duas instâncias de publicação. NPR-21943: Hotfix do Granite-19564
* Problemas de desempenho com o resolvedor de sessão/recurso compartilhado org.apache.sling.i18n. NPR-23390: Hotfix do Sling-7543
...
* Ao fornecer uma at.js ou mbox.js personalizada, o script de inclusão é gravado na página como texto em vez das tags HTML. NPR-22441: Hotfix do CQ-4203691
...
* A publicação de uma página com conteúdo direcionado não tem recursos relacionados. NPR-23064: Hotfix do CQ-4227119
...
* O iParsys cria a mudança de posição depois de desmarcar a caixa de seleção cancelar/desativar herança. NPR-21905: Hotfix do CQ-4230951
...
* Problemas com o seletor de mês do Calendário Coral quando o intervalo de datas &quot;mín&quot; e &quot;máx&quot; está definido. NPR-22716: Hotfix do CUI-7187
...
* Autenticação básica de cabeçalho SOAP corrigida.
* Adição de suporte para atributo de título para hiperlink.
* Habilitar o Identificador de PDF/UA na árvore de acessibilidade do Forms Designer.
...
* A senha é visível em texto sem formatação no campo de senha de inspeção das páginas de Configuração dos Conectores ECM. NPR-22508: Hotfix do CQ-4236065
...
* Não é possível sincronizar o LDAP devido à versão incorreta de cglib-nodep. NPR-22493: Hotfix do CQ-4234099
...
* A codificação de entrada SOAP UTF-8 no Adaptive Forms retorna erros com página distorcida. NPR-20105: Hotfix do CQ-4222669
* O componente de Contêiner do AEM Forms não estará disponível para edição depois que o formulário incorreto for configurado na página Sites. Hotfix do CQ-4237456
...
* Os Forms modificados após a publicação da página não serão publicados novamente na publicação do site. NPR-23013: Hotfix do CQ-4236566

#### Integração de backend {#backend-integration-1}

* A autenticação básica OOTB para serviços SOAP não funciona para autenticação básica na integração do FDM. NPR-23238: Hotfix do CQ-4241308
...
* Baixar o log do servidor usando o Workbench funciona somente para um servidor, enquanto para o outro servidor não funciona. NPR-21079: Hotfix do CQ-4229842
...
* Erro encontrado na inicialização ao iniciar o Agendador. NPR-22990
...
* O Identificador de PDF/UA está ausente quando salvo como PDF do Designer. NPR-23137
* Objetos de caminho, por exemplo: bordas, sublinhados e hiperlinks, não são marcados quando salvos como PDF do Designer. NPR-23136
* O objeto de imagem não tem caixa delimitadora quando salvo como PDF do Designer. NPR-23134
* Os hiperlinks incorporados definidos no Designer não são marcados nem têm texto alternativo quando salvos como PDF do Designer. NPR-23133
...
* Correção de problemas de interface do usuário na página de verificação de licença para obter melhor experiência.
* Ativação do suporte à tarefa de fluxo de trabalho OSGi no aplicativo AEM Forms.
...
* Solução de vários problemas de manipulação de pastas com APIs de ativos. NPR-20569
...
* O nome do fluxo de trabalho digitado deve estar disponível na caixa Notificação ao publicar página por meio de Gerenciar publicação. NPR-20046: Hotfix do CQ-4221586
...
* A filtragem por tags nas páginas de coleção no console Produtos não funciona. NPR-20292: Hotfix do CQ-4224023
...
* As notificações por email não são acionadas para nenhuma das atividades relacionadas ao moderador em componentes publicados. NPR-20122
* Paginação em branco sem resultados é exibida para usuários anônimos da comunidade. NPR-20136: Hotfix do CQ-4220738
...
* CommunityUserOperationExtension executa uma exceção de conversão de classe. NPR-20532: Hotfix do CQ-4224385
...
* A reativação do segmento após qualquer modificação falhar e gerar um erro. NPR-20054: Hotfix do CQ-4218401
...
* requestAttributes não tem a definição removida para as outras instruções data-sly-resource. NPR-20639: Hotfix do 4226206
* Correção de problemas com solicitações curl Head para ativos OOTB no AEM. NPR-20781: Hotfix do CQ-4221520

### Projetos {#projects-1}

* O acesso a diferentes projetos do console de Projetos leva mais tempo para carregar. NPR-20314
* A instalação do AEM 6.3.0.1 remove o armazenamento de chaves do usuário dam-update-service. NPR-20018
* Em algumas implantações personalizadas, os usuários que tentam selecionar um destinatário no módulo addTask demoram mais para preencher a lista no selecionador de usuário. NPR-20283: Hotfix do CQ-4224193

### Interface do usuário {#user-interface-3}

* Colorfield está definido como &quot;sempre obrigatório&quot; apesar dos atributos na caixa de diálogo. NPR-19702
...
* Não é possível definir o valor do componente de comutação entre verdadeiro/falso. NPR-21111
...
* Ativação do suporte à tarefa de fluxo de trabalho OSGi no aplicativo AEM Forms. CQ-4222638
...
* Renderização de tempo do lado do servidor substituída pelo componente de tempo de base.
* Ativação da opção Publicar tags do AEM no Brand Portal a partir do console tagadmin/tagging.
...
* Mapear dois campos com a mesma propriedade com diferentes tipos de campo de propriedade gera um erro interno. NPR-19462: HF do CQ-4216828
* dc: title e dc: description não são alteradas para um valor de vários campos no    crx /de. NPR-19570: HF do CQ-4209086
* O Visualizador dinâmico carrega a representação de vídeo de qualidade mais baixa para testar a experiência de reprodução de vídeo no modo de autor. NPR-19004
...
* Ao selecionar um ativo na exibição de coluna e clicar em editar, a página retorna um erro. NPR-20301: Hotfix do CQ-4224052
* A exibição da Expiração do ativo está desativada ao atravessar fusos horários positivos e negativos. NPR-20329: Hotfix do CQ-4219333
...
* O campo Mecanismo de definição de metas no assistente de configuração é definido como ContextHub (AEM) em vez de Adobe Target. NPR-19320: HF do CQ-4218465
...
* Os valores suspensos de campo multicomposto não são preenchidos novamente devido ao código de alteração no Sidekick.js depois de atualizar a instância para AEM 6.1SP2-CFP3. NPR-19450: HF do CQ-4194771
...
* Não é possível gerar visualização para projetos de Tradução. NPR-19289
...
* As notificações por email não funcionam para nenhuma das atividades relacionadas ao moderador em componentes publicados. NPR-19767: HF do CQ-4218060
...
* O fluxo de trabalho Enviar para JEE não retorna o parâmetro de saída do JEE para o AEM e lança o erro &quot;Ignorando porque o valor não é primitivo&quot;. NPR-20265
...
* Ao configurar o Google reCaptcha usando o Cloud Service, em ambientes que exigem configuração de org.apache.http.proxyconfigurator para conexões externas, as chamadas de POST não parecem passar pelo PROXY. NPR-20454
* O Forms adaptável baseado em esquemas XSD envia valores XML com falha para campos numéricos em instalações com localidades que têm um separador decimal diferente de &quot;.&quot; resultando em erros. NPR-20444
...
* Caracteres UTF-8 como saída do serviço SOAP não são copiados corretamente nos campos do Forms adaptável para localidades em outros idiomas. NPR-20238: NPR-20103

#### Gerenciamento de correspondência {#correspondence-management-1}

* Copiar Colar conteúdo do arquivo do Word perde a cor do conteúdo e a fonte no Editor de texto. NPR-19521

#### Serviços do Assembler {#assembler-services}

* Discrepância entre os resultados de Acrobat e AEM durante uma verificação de conformidade de um documento para o formato PDFA-1b. NPR-19280

#### Fluxo de trabalho {#workflow-2}

* Etapa Assinar Fluxo de Trabalho para permitir que chamadas http se conectem por meio do proxy. NPR-20529

#### Formulários HTML5 {#html-forms-4}

* Adição de suporte para o evento preSubmit. NPR-20604
...
* O campo de texto multilinha (TextArea) não retém o novo caractere de linha ou quebra no texto após enviar dados no espaço de trabalho do HTML. NPR-20085
...
>[!NOTE]
>
>Instalar o pacote incorporado do LiveCycle listado em [Versões do AEM Forms](aem-forms-releases.md) para corrigir o problema.

#### Serviços padrão {#standard-services}

* O docConvertor não suporta transparências de nivelamento no PDF e não produz um PDF/A. NPR-16228: Hotfix do CQ-4214488

#### Núcleo {#core-2}

* Quando o servidor do AEM Forms em execução em uma configuração de cluster no aplicativo JBoss é interrompido, o servidor do aplicativo é desconectado do banco de dados. Isso pode levar a problemas de corrupção de dados. NPR-19724
...
* Configuração simplificada para serviços de conteúdo inteligentes
...
* Aprimoramento da usabilidade de páginas com conteúdo pesado na exibição de coluna
* Maior conformidade com a convenção de nomenclatura de página para páginas com títulos longos
* Experiência de personalização de campanha aprimorada na interface para toque
* Correções para vários problemas de sobreposição de projetos
...
* Requisito para a auditabilidade das alterações de permissão do usuário no AEM. NPR-17061
* Conexões persistentes com Cloud Services DM que causam exceções de &quot;Muitos arquivos abertos&quot;. CQ-4211407

### Ativos {#assets-12}

* Problemas de usabilidade com a configuração dos serviços de conteúdo inteligente usando opções diferentes. NPR-18200: Hotfix do CQ-4201557
* Vazamento de recursos em fluxos binários para o armazenamento de dados S3. NPR-18041: Hotfix do CQ-4209506
...
* A exceção de ponteiro nulo ocorre quando um ativo é movido de sua página de propriedades. CQ-4204251
...
* Em uma pasta de ativos baixados, se ocorrer um conflito ao extrair o conteúdo do arquivo ZIP, a opção padrão aparecerá como Criar versão em vez de Manter ambos. CQ-97800
...
* Problemas ao navegar em uma página com conteúdo excessivo na exibição em coluna na interface para toque. NPR-17799: Hotfix do CQ-4199611
...
* Problemas com a criação de página na interface do usuário de toque no AEM 6.3 implantada no Jboss EAP 6.4. NPR-17589: Hotfix do CQ-4210137
* O provedor de status do workflow faz com que a instância seja bloqueada quando grupos aninhados estão presentes. NPR-17556: Solicitação CFP do CQ-4202056
...
* Em um fluxo de trabalho não transitório, o histórico do processo e as alterações dos metadados feitas antes de uma etapa do processo externo não são persistentes. NPR-17848: Hotfix do GRANITE-17757
* Os valores dos campos de diálogo do fluxo de trabalho não são persistentes no nó do item de trabalho. NPR-17734: Hotfix do CQ-4210369
...
* A reestruturação do pods no módulo de projetos o torna menos configurável. CQ-4209859
* O caminho dos ativos altera o caminho para os respectivos sites localizados quando as páginas são adicionadas ao trabalho de tradução. CQ-4206007
...
* Correções nos módulos de texto do gerenciamento de correspondências, pré-visualizações de cartas e inicialização programática da interface do usuário do gerenciamento de correspondência de criação.
...
* Margem adicional acima e abaixo de uma lista com marcadores é visível no editor de texto, mas não na representação de HTML e PDF. NPR-18126
* Quando o envio de HTML usou o método POST, a interface de criação de correspondência não é iniciada. NPR-18202
...
* Ao selecionar para processar arquivos usando um workflow, ao criar uma pasta de observação, o menu suspenso de seleção do modelo de workflow aparece truncado. NPR-17566

**Formulários HTML5**

* O AEM Forms não captura eventos de auditoria para o campo de assinatura de rabisco. NPR-15286

**Forms Manager**

* A interface do usuário do AEM Forms lista todos os ativos na primeira ordem mais antiga. Os usuários não podem reordenar os ativos na primeira ordem mais recente. NPR-18450
...
* A variável TaskContext não é preenchida para processos AEM Forms. NPR-18199

**Segurança de documentos**

* O Microsoft Excel e o Microsoft PowerPoint demoram muito mais para abrir os documentos protegidos com o AEM Document Security Extension for Microsoft Office. CQ-4212358
* Quando uma nova política é criada e uma política com o mesmo nome da existente, ocorre um erro interno do servidor. NPR-18247
...
>[!NOTE]
>
>Se estiver instalando versões 6.3.3.0 ou posteriores, execute o procedimento abaixo para definir as configurações do servidor de aplicativos JBoss. Se estiver instalando o 6.3.3.0 no servidor do AEM Forms em execução nos servidores de aplicativos Oracle WebLogic ou IBM WebSphere, nenhuma configuração adicional será necessária. Para obter mais detalhes, consulte [Notas de versão do AEM 6.3.3.0](https://helpx.adobe.com/br/experience-manager/6-3/release-notes/sp3-release-notes.html).
...
* Os seguintes erros e avisos podem ocorrer durante a instalação do AEM CFP 6.3.3.x e podem ser ignorados com segurança:
   ...
   * &#42;ERRO&#42; [OsgiInstallerImpl] com.adobe.cq.social.cq-social-jcr-provider [com.adobe.cq.social.provider.jcr.impl.SpiSocialJcrResourceProviderImpl(2174)] Tempo limite de espera pela conclusão não registrada da alteração do registro. CQ-4209974.
   ...
   * &#42;ERRO&#42; [FelixDispatchQueue] com.day.cq.dam.cq-dam-core FrameworkEvent ERROR (org.osgi.framework.ServiceException): O fabricante do serviço retornou como nulo. (Componente: com.day.cq.dam.handler.standard.ps.PostScriptHandler))
...
>[!NOTE]
>
>Caso seja afetado por qualquer um desses dois problemas, entre em contato com [Atendimento ao cliente AEM](https://helpx.adobe.com/br/marketing-cloud/contact-support.html).
...
* Abra adobe-livecycle-cq-author.ear/cq.war/WEB-INF/web.xml para edição:
   ...
   * Após a alteração acima, init-param deve se parecer com:
...
|----|-----|-----|-----|
| Integração do Assets e do Adobe Creative Cloud | [O compartilhamento de pastas do AEM para a Creative Cloud](https://helpx.adobe.com/br/experience-manager/6-3/sites/administering/using/creative-cloud.html) foi introduzido no AEM 6.2 como uma forma de fornecer aos usuários criativos acesso aos ativos do AEM. Um novo recurso lançado no aplicativo Creative Cloud, o Adobe Asset Link, fornece uma experiência de usuário muito melhor e acesso mais eficiente a ativos do AEM diretamente do Photoshop, do InDesign e do Illustrator.<br /> O Adobe não fará mais melhorias no recurso de compartilhamento de pastas. Embora o recurso esteja incluído no AEM, os clientes são altamente recomendados a usar a substituição. | Adobe Asset Link ou Aplicativo de desktop. Para obter mais informações, consulte [Integração de Creative Cloud com AEM](https://helpx.adobe.com/br/experience-manager/6-3/assets/using/aem-cc-integration-best-practices.html) artigo. | AEM 6.3.3.x |
